...
class AuthProvider with ChangeNotifier {
  ...
  Future<String> getPersistentDeviceId() async {
    ...
    if (deviceId == null) {
      deviceId =
          'device_${DateTime.now().millisecondsSinceEpoch}_${Random().nextInt(1000000)}';
  ...
  Future<bool> requestProfileUpdateOtp(
      {required String documentId, required String newMobile}) async {
  ...
  Future<bool> updateDeviceToken({
    ...
    try {
      debugPrint(
          "🔄 [AuthProvider] Updating device token for security sync...");
      ...
      if (docSnapshot.exists) {
        ...
        if (previousToken != null &&
          ...
          debugPrint(
              "⚠️ [AuthProvider] New device detected! Resetting face verification.");
        ...
        debugPrint(
            "✅ [AuthProvider] Device token successfully bound to Firestore session.");
        return true;
      }
      return false;
    } catch (e) {
