...
class BookingProvider extends ChangeNotifier {
  ...
  // 💡 Added recently: Prevent map centering (Pickup location change) during auto zoom via code.
  ...
  // Choose on Map State
  bool isChoosingOnMap = false;
  bool isChoosingPickup = true;
  String hoveredAddress = "Fetching address...";
  int currentDropIndex = 0;

  void enableChooseOnMap({required bool isPickup, int dropIndex = 0}) {
    isChoosingOnMap = true;
    isChoosingPickup = isPickup;
    currentDropIndex = dropIndex;
    notifyListeners();
  }

  void disableChooseOnMap() {
  ...
  // =========================================================================
  // 💡 2. Function to delete a saved location
  // =========================================================================
  ...
  Future<void> updateHoveredAddress(LatLng location) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        if (data['status'] == 'OK' && data['results'].isNotEmpty) {
          hoveredAddress = data['results'][0]['formatted_address'];
          notifyListeners();
        }
      }
    } catch (e) {
      hoveredAddress = "Location selected";
      notifyListeners();
    }
  }
  ...
  Future<void> calculateRoute() async {
    ...
    LatLng destination = validDrops.last; // Last drop
  ...
  // =========================================================================
  // 💡 3. Function to clear all Pickups/Drops when navigating back from the page
  // =========================================================================
  void resetBookingData() {
    fetchSavedLocations(); // Fetch saved locations when resetting data
    loadRecentLocations(); // Load recent locations
