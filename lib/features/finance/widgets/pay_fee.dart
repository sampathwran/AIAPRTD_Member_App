...
Future<void> handlePayMembershipFee(
    BuildContext context, double balance) async {
  ...
  if (balance < monthlyFee) {
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(
            'Insufficient savings. You need LKR $monthlyFee to pay the fee.')));
    return;
  }
  ...
  bool? confirm = await showDialog<bool>(
    ...
    builder: (context) => AlertDialog(
      ...
      content: Text(
          "Are you sure you want to deduct LKR $monthlyFee from your savings to pay this month's fee?"),
      actions: [
        TextButton(
    ...
    try {
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Membership Fee paid successfully!')));
      }
    } catch (e) {
      debugPrint("Error paying fee: $e");
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text('Failed to pay fee. Try again later.')));
      }
    }
  }
}
