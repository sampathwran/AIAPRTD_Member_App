...
class MyAdsPage extends StatelessWidget {
  ...
  void _showPriceDropDialog(
      BuildContext context, String adId, String currentPrice) {
            ...
            onPressed: () async {
              if (priceController.text.isNotEmpty) {
                final res = await context
                    .read<AdsProvider>()
                    .updateAdPrice(adId, priceController.text);
                if (context.mounted) {
    ...
    return Scaffold(
      ...
      body: StreamBuilder<QuerySnapshot>(
        ...
        builder: (context, snapshot) {
          ...
          if (docs.isEmpty)
            return const Center(child: Text("You haven't posted any ads yet."));
                  ...
                  child: Row(
                    children: [
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            ...
                            const SizedBox(height: 4),
                            Text(
                                "Posted: ${createdAt != null ? DateFormat.yMd().format(createdAt.toDate()) : 'Unknown'}",
                                style: const TextStyle(
                      ...
                      PopupMenuButton<String>(
                            ...
                            Navigator.push(
                                ...
                                MaterialPageRoute(
                                    builder: (_) => AdDetailsPage(
                                        adData: data, adId: docs[index].id)));
                          } else if (value == 'drop_price') {
                            _showPriceDropDialog(
                                context, docs[index].id, priceStr);
                          } else if (value == 'mark_sold') {
                            final res = await context
                                .read<AdsProvider>()
                                .markAdAsSold(docs[index].id);
                        ...
                        itemBuilder: (context) => [
                          const PopupMenuItem(
                              value: 'view', child: Text("View Ad Details")),
                          const PopupMenuItem(
                              value: 'drop_price', child: Text("Drop Price")),
                          if (status == 'approved')
                            const PopupMenuItem(
                                value: 'mark_sold',
