...
class PaymentHistoryTab extends StatefulWidget {
  ...
  const PaymentHistoryTab(
      {super.key, required this.memberData, required this.isDark});
...
class _PaymentHistoryTabState extends State<PaymentHistoryTab> {
  ...
  int _calculateArrears(DateTime? lastPaidPeriod) {
    ...
    int arrears = ((currentYear - lastPaidPeriod.year) * 12) +
        (currentMonth - lastPaidPeriod.month);
  ...
  Widget build(BuildContext context) {
    final List<dynamic> paymentHistory =
        widget.memberData['payment_history'] ?? [];
    final List<dynamic> pendingPayments =
        widget.memberData['pending_payments'] ?? [];
          ...
          if (pendingPayments.isNotEmpty) ...[
            ...
            ...pendingPayments.map((pending) {
              ...
              if (pendingMap.containsKey('months')) {
                List<dynamic> mList =
                    pendingMap['months'] is List ? pendingMap['months'] : [];
                pendingMonthText = mList.join(', ');
                if (pendingMap.containsKey('paymentDate') &&
                    pendingMap['paymentDate'] != null) {
                  try {
                    DateTime pd =
                        DateTime.parse(pendingMap['paymentDate'].toString());
              ...
              return Container(
                ...
                child: ListTile(
                  ...
                  title: Text("$pendingMonthText $yearText Verification",
                      style: TextStyle(
                          fontWeight: FontWeight.bold,
                          fontSize: 15,
                          color: textColor)),
          ...
          if (paymentHistory.isEmpty)
            ...
            ListView.separated(
              ...
              itemBuilder: (context, index) {
                ...
                String periodText =
                    "${recordMap['month']?.toString() ?? '-'} ${paidPeriod.year}";
                    ...
                    subtitle: Padding(
                      ...
                      child: Row(
                        children: [
                          ...
                          Text(
                              (recordMap['paymentDate']?.toString() ??
                                      recordMap['date']?.toString() ??
                                      '-')
                                  .split('T')
                                  .first
                                  .split(' ')
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          Text("Your payment records will appear here once approved.",
