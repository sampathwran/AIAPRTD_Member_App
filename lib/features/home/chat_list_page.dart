...
class ChatListPage extends StatelessWidget {
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const Text("Chats will appear here when you have an active trip.",
  ...
  Widget _buildChatTile(
      BuildContext context, QueryDocumentSnapshot tripDoc, String myMemberId) {
    var data = tripDoc.data() as Map<String, dynamic>;
    String tripId = tripDoc.id;

    bool isDriver = data['acceptedBy'] == myMemberId;
    String otherUserId = isDriver
        ? (data['memberId'] ?? 'Unknown Passenger')
        : (data['acceptedBy'] ?? 'Unknown Driver');
        ...
        return ListTile(
          ...
          leading: Stack(
            children: [
              CircleAvatar(
                backgroundColor:
                    isDriver ? Colors.green.shade100 : Colors.blue.shade100,
                radius: 24,
                child: Icon(
                  isDriver ? Icons.person : Icons.directions_car,
                  color:
                      isDriver ? Colors.green.shade700 : Colors.blue.shade700,
                ),
              ),
              if (hasUnread)
                Positioned(
