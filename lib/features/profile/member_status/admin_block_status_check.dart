Map<String, dynamic> checkAdminBlockStatus(Map<String, dynamic>? memberData) {
  ...
  final String blockStatus =
      memberData['adminBlockStatus']?.toString().toLowerCase() ?? 'none';
  final String blockReason =
      memberData['adminBlockReason']?.toString() ?? 'Account blocked by Admin';

  // Legacy status fallbacks
  final String adminApproval =
      memberData['adminApproval']?.toString().toLowerCase() ?? '';
  final String mainStatus =
      memberData['profile_status']?.toString().toLowerCase() ??
          memberData['status']?.toString().toLowerCase() ??
          '';
  final String legacyReason = memberData['inactiveReason']?.toString() ??
      memberData['rejectionReason']?.toString() ??
      'Account has been restricted by Admin.';

  if (blockStatus == 'temporary' || blockStatus == 'permanent') {
    return {
      'isActive': false,
      'reason': blockReason.isNotEmpty
          ? blockReason
