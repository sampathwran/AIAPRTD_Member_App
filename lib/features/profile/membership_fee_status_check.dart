...
Map<String, dynamic> checkMembershipFeeStatus(Map<String, dynamic>? data) {
  ...
  if (!data.containsKey('payment_history') || data['payment_history'] == null) {
    return {
      'isFeePaidValid':
          true, // This line allows going online immediately after opening the app!
      'reason': '',
    };
  }
  ...
  final String currentMonthName =
      monthNames[now.month - 1]; // Current month name
  ...
  final List<dynamic> allPaymentsToCheck = [
    ...paymentHistory,
    ...pendingPayments
  ];
  ...
  for (var payment in allPaymentsToCheck) {
    if (payment is Map) {
      ...
      print(
          "🔍 [FeeCheck] Checking Payment -> months: $monthsToCheck | year: '$pYear' | reason: '$pReason' | isMembership: $isMembershipPayment | currentMonthName: '${currentMonthName.toLowerCase()}' | currentYear: '$currentYearStr'");

      if (monthsToCheck.contains(currentMonthName.toLowerCase()) &&
          (pYear == currentYearStr ||
              pYear
                  .isEmpty) && // Pending payments sometimes might not have year set properly immediately
          isMembershipPayment) {
